...
<link href="<?php echo base_url(); ?>assets/desain/css/style.css?<?php echo time(); ?>" rel="stylesheet">
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbar">
            <ul class="nav navbar-nav navbar-right" style="z-index: 1000">
                <li><a class="btn" onclick="return simpan_akhir();"><i class="glyphicon glyphicon-stop"></i> Selesai Ujian</a></li>
            </ul>
        </div>
    </div>
</nav>
...
<script type="text/javascript">
    ...
    simpan = function() {
        ...
        $.ajax({    
            type: "POST",
            url: base_url+"siswa/mulai_tugas/simpan_satu/"+id_tugas,
    ...
    selesai = function() {
        var f_asal  = $("#_form");
        var form  = getFormData(f_asal);
        simpan_akhir(id_tugas);
        window.location.assign("<?php echo base_url(); ?>siswa/sudah_selesai_ujian/"+id_tugas); 
    ...
    simpan_akhir = function() {
        simpan();
        if (confirm('Anda yakin ingin mengakhiri tes ini?')) {
            simpan();
            $.ajax({
                type: "GET",
                url: base_url+"siswa/mulai_tugas/simpan_akhir/"+id_tugas,
                beforeSend: function() {
                    $('.ajax-loading').show();    
                },
                success: function(response) {
                    console.log('masuk pa eko')
                        window.location = ("<?php echo base_url(); ?>siswa/konfirmasi_tugas/PG/"+id_tugas); 
                }
            });
